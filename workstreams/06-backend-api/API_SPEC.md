...
Authorization: Bearer ***       // 登录后携带(opaque bearer token)
X-App-Version: 1.0.0
...
> 所有响应（成功和错误）均在响应头中返回 `X-Request-Id`。若请求携带了 `X-Request-Id`，后端原样返回；否则后端生成。
...
> 路由与蓝图 `/api/v1/notifications/*` 对齐。分页使用 cursor/limit（见 §2.3）。
...
> 仅 seeker 可撤回自己处于 `submitted`/`viewed`/`shortlisted` 状态的投递。`hired` 状态不可撤回。撤回操作天然幂等：重复撤回同一投递，后端返回业务错误（如 `ALREADY_WITHDRAWN`）或 no-op（200 返回当前状态），不产生副作用。
...
> 不分页。返回单个聚合对象（含分类、岗位、创业支持、服务等区块）。
